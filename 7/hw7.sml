...
fun preprocess_prog e =
    case e of
    LineSegment(x, y, w, z) => 
        if real_close(x, w) andalso real_close(y, z) 
        then Point(x, y)
        else 
            if ((x > w) andalso not (real_close(x, w))) 
            then LineSegment(w, z, x, y)
            else 
                if real_close(x, w) andalso (y > z) andalso not (real_close(y, z))
                then LineSegment(w, z, x, y)
                else e
