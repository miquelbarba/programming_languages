...
let 
    val Line(a,b) = (eval_prog (preprocess_prog (Shift(1.0, 2.0, Line(5.0, 5.0))), []))
    val Line(c,d) = Line(5.0, 2.0) 
in
    real_equal(a,c) andalso real_equal(b,d)
end;

let 
    val VerticalLine(a) = (eval_prog (preprocess_prog (Shift(2.0, 1.0, VerticalLine(9.0))), []))
    val VerticalLine(c) = VerticalLine(11.0) 
in
    real_equal(a,c)
end;

let 
    val LineSegment(a, b, a1, b1) = (eval_prog (preprocess_prog (Shift(3.0, 4.0, LineSegment(0.0, 1.0, 2.0, ~1.0))), []))
    val LineSegment(c, d, c1, d1) = LineSegment(3.0, 5.0, 5.0, 3.0) 
in
    real_equal(a,c) andalso real_equal(b, d) andalso real_equal(a1, c1) andalso real_equal(b1, d1)
end;

let 
    val exp = Let("a", LineSegment(1.0, 2.0, 3.2, 4.1), LineSegment(2.0, 4.0, 3.0, 4.1))
    val Let(x, LineSegment(a, b, c ,d), LineSegment(e, f, g, h)) = preprocess_prog(exp)
in
    x = "a" andalso real_equal(a, 1.0) andalso real_equal(b, 2.0) andalso real_equal(c, 3.2)
    andalso real_equal(d, 4.1) andalso real_equal(e, 2.0)
end;

let 
    val LineSegment(a,b,c,d) = preprocess_prog (LineSegment(4.2,5.0,4.2,3.0))
    val LineSegment(e,f,g,h) = LineSegment(4.2, 3.0, 4.2, 5.0)
in
...
let 
    val Point(a, b) = eval_prog(preprocess_prog (Shift(3.0, 4.0, LineSegment(1.0, 1.0, 1.0, 1.0))), [])
in
    real_equal(a, 4.0) andalso real_equal(b, 5.0)
end;
