...
class Line < GeometryValue
  ...
  def intersectPoint p
    real_close(p.y, m * p.x + b) ? p : NoPoints.new
  end    
...
class VerticalLine < GeometryValue
  ...
  def intersectPoint p
    real_close(p.x, x) ? p : NoPoints.new
  end  
...
class Var < GeometryExpression
  ...
  def eval_prog env 
    var = env.find {|name, value| name == @s}
    if var.nil?
