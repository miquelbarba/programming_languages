...
(define f2_sumall (eval-exp (fun "sumall" "nb" (ifgreater (var "nb")
                                                          ...
                                                          (int 0)))))
(define mupl-sum-first-n (fun "sum-first-n" "n"
                              (ifgreater (var "n") (int 0)
                                         (add (var "n")
                                              (call (var "sum-first-n") (add (var "n") (int -1))))
                                         (int 0))))
...
; 2
  (test-equal? "Testing ifgreater 1 2"
               (int 42)
               (eval-exp (ifgreater (int 1) (int 2) (add (var "crashifevaluated") (int 3)) (int 42))))
  
  (test-equal? "Testing ifgreater 2 1"
               (int 42)
               (eval-exp (ifgreater (int 2) (int 1) (int 42) (add (var "crashifevaluated") (int 3)))))
  
  
  (test-equal? "Testing call"
               (int 45)
               (eval-exp (call f2_1 (int 3))))
    
  (test-equal? "Testing mlet+call"
              (int 45)
              (eval-exp (call f2_2 (int 3))))
  ...
  (test-equal? "Testing recursive function"
                (int 55)
                (eval-exp (call f2_sumall (int 10))))  
  

  (test-equal? "Recursive function"
               (eval-exp (call mupl-sum-first-n (int 20)))
               (int 210))    
...
#| 
(define addtwo (fun "addone" "x" (add (var "x") (int 2))))
(call (call mupl-map addtwo) (aunit))
    )
 
   
(call
 (call
  (fun #f "f" (fun "helper" "xs" (apair (call "f" (fst (var "xs"))) (call "helper" (snd (var "xs"))))))
  (fun "addone" "x" (add (var "x") (int 2))))
